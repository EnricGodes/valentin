---
title: "Porsche 964, el primer amb ABS i tracció integral"
slugSquarespace: "porsche-964"
slugFinal: "porsche-964"
slugOriginal: "porsche-964"
date: 2021-12-09T18:19:00.252000Z
lang: ca
categories: ["iconos"]
excerpt: "La tercera generació del 911 va ser un repte per a Porsche: el 85% de les peces eren noves, en una carrosseria estreta i compacta que encara avui convenç a la carretera."
metaDescripcion: "La tercera generació del 911: tracció 4, ABS, direcció assistida i un 85 % de peces noves. Versions, motors Turbo i RS, i 63.762 unitats."
metaTitulo: "Porsche 964: versions, motoritzacions i història · Valentín Motors"
author: "Enric Godes"
squarespaceId: "6203a0188e0d4e55e34569d4"
originalUrl: "https://www.valentinmotors.es/magazine/porsche-964"
ctas: []
---
![Porsche 911 964, el primer amb ABS i tracció integral](/img/magazine/350c9319-porsche-964.jpg)

*Porsche 911 964, el primer amb ABS i tracció integral*

La tercera generació del 911, coneguda com a 964, va ser un repte per a la marca. Apareix després de diversos anys amb la generació G i després dels innovadors 928 i 944 amb motor davanter i tracció posterior.

> El 964 és un buf d’aire nou, i és el model amb un total del **85% de les peces noves**; això sí, en carrosseria estreta, compacta, que a dia d’avui continua sent molt atractiva a la carretera.

## Principals novetats del Porsche 911 model 964

Les novetats que van fer del model un èxit: tracció integral, sistema d’ABS i direcció assistida, alguns d’aquests elements recuperats del model 959 i pensats per millorar la conducció i aconseguir més seguretat i confort.

Es presenta el 1988 amb un motor 3.6 de 250 CV de doble encesa per aconseguir més fiabilitat, es corregeix l’efecte aerodinàmic amb un spoiler posterior retràctil i en versió 4. El 1989 apareix la versió Carrera 2.

![Porsche 964 Turbo negre en marxa per una carretera de muntanya](/img/magazine/7d05824d-nika-qufarashvili-3NZ2LuVpKeI-unsplash.jpg)

Amb el 964 tenim una explosió de colors: fins a 58 colors en diferents versions, alguns recuperats d’altres versions però la majoria nous — 80K indischrot, 40D espressobraun, 37B taubenblau i un llarg etcètera. Alhora, podem parlar d’un vehicle amb **pensament ecològic**: per primera vegada la pintura és **a l’aigua**, sense dissolvents, i el 80% dels plàstics es poden **reciclar**.

És un model diferenciador que va marcar una etapa nova a la marca, etapa de prosperitat que es reflecteix en el model **30 Aniversari**: només 911 unitats, amb una carrosseria més pròxima a les versions Turbo però sense aleró posterior, en un exclusiu Viola metal·litzat exterior i Gris Rubicon interior.

## Les motoritzacions del model 964

Parlant de les versions Turbo, cal destacar les diferents motoritzacions: Turbo 3.3 amb 320 CV, Turbo S 3.3 amb 381 CV i, el 1993, Turbo 3.6 amb 360 CV. Però hi ha diferents versions que han fet d’aquest model un esperit d’esportivitat: Speedster, RS de curses i de carrer, vehicles austers i preparats per a la competició, amb motoritzacions de 275 CV i tracció només posterior.

![Porsche 911 Turbo negre aparcat de perfil davant d'una façana de maó](/img/magazine/6328b22b-valentin-b-kremer-mhrZmA_2eEg-unsplash.jpg)

Un total de 63.762 unitats han fet d’aquest model un dels preferits pels amants de la marca. Senzill, lleuger, ràpid i fiable són les seves característiques més grans, que fan les delícies dels propietaris.

> **El 964 és un dels millors models per introduir-se al món Porsche, un autèntic youngtimer.**
